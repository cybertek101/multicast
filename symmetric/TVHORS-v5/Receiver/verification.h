...
int Verification(char *orgMsg, lightType *signatures, struct pmuInfo *pmu);
...
int Verification(char *orgMsg, lightType *signatures, struct pmuInfo *pmu) {
	...
	double diff = timeSecons(local_recv_time) - pmu->senderTimeSecond;
	double decTime = diff + ENDTOEND_DELAY - pmu->currEpoch * EPOCH_DURATION;
	...
	if (decTime >= ADV_CALCULATION) {
		...
		memcpy(newMsg, &pmu->a, sizeof(uint32_t));
		memcpy((newMsg + sizeof(uint32_t)), orgMsg, MSG_LENGTH);
		memcpy((newMsg + sizeof(uint32_t) + MSG_LENGTH), &pmu->currSalt, sizeof(struct saltType));
		...
		for (i=0; i<*(size); ++i) {
			if (!verifySALT(&pmu->currSalt, pmu->currEpoch, pmu->lastSalt) || !verifySAGE(&signatures[i], *(mappingSet + i), pmu->currEpoch, pmu->lastSalt, pmu->lastLight))
				return -1;

			memcpy(&pmu->lastLight[*(mappingSet + i)], &signatures[i], sizeof(struct lightType));
		}
		pmu->lastSalt[pmu->currEpoch] = pmu->currSalt;
	}
	free(newMsg);
	free(size);
