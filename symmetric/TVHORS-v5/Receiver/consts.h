...
#define PDC_IN_PORT				5000	// Port; from pmu to pdc
#define PDC_OUT_PORT			6000	// Port; from pdc to labview
...
typedef struct pmuInfo {
	...
	double senderTimeSecond;
	uint32_t currEpoch;										// Index of the current epoc
	uint32_t a;												// Index of the msg to be sent
	saltType currSalt;										// Received salt k(c)

	struct in_addr addr;									// IP address of the sender - pmu
	int scrPort;											// Source port number
} pmuInfo;
